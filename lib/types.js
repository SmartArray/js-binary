...
module.exports.rgb = {
	write: function (d, data, path) {
		if (!d) throw new TypeError('Expected non-empty value for rgb color at ' + path + ', got ' + d)
		
		var buffer

		if (d.indexOf('rgb(') === 0) {
			var splitted = d.match(/^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/)
			if (!splitted) throw new TypeError('Expected valid rgb value at ' + path + ', got ' + d)

			var r = splitted[1]
			var g = splitted[2]
			var b = splitted[3]

			if (r > 255 || g > 255 || b > 255) throw new TypeError('Invalid rgb components at ' + path + ', got ' + d)

			buffer = Buffer.from([r, g, b])

		} else if (d.indexOf('#') === 0) {
			var rest = d.substr(1)

			if (rest.length === 3) {
				var full = rest.split('').map(w => parseInt('' + w + w, 16))
				buffer = Buffer.from(full)

			} else if (rest.length === 6) {
				buffer = Buffer.from(rest, 'hex')

			} else {
				throw new TypeError('Expected three or six digit rgb value at ' + path + ', got ' + d);
			}

		} else {
			throw new TypeError('Expected valid rgb value at ' + path + ', got ' + d)
		}

		exports.Buffer.write(buffer, data, path)
	},

	read: function (state) {
		var buffer = exports.Buffer.read(state)
...
module.exports.rgba = {
	write: function (d, data, path) {
		if (!d) throw new TypeError('Expected non-empty value for rgba color at ' + path + ', got ' + d)
		
		var buffer
		var alpha = null

		if (d.indexOf('rgba(') === 0) {
			var splitted = d.match(/^rgba\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3}),\s*(\d*(?:\.\d+)?)\)$/);
			if (!splitted) throw new TypeError('Expected valid rgba value at ' + path + ', got ' + d)
			...
			if (r > 255 || g > 255 || b > 255) throw new TypeError('Invalid rgb components at ' + path + ', got ' + d)
			if (a < 0 || a > 1) throw new TypeError('Invalid rgba alpha component at ' + path + ', got ' + d)

			buffer = Buffer.from([r, g, b])
			alpha = parseFloat(a)

		} else {
			throw new TypeError('Expected valid rgba value at ' + path + ', got ' + d)
		}

		exports.Buffer.write(buffer, data, path)
		exports.float.write(alpha, data, path)
	},

	read: function (state) {
		var buffer = exports.Buffer.read(state)
		var alpha = exports.float.read(state)

		return 'rgba(' +
			buffer[0].toString(10) + ', ' + 
			buffer[1].toString(10) + ', ' + 
			buffer[2].toString(10) + ', ' + 
			alpha + ')'
	}
}
